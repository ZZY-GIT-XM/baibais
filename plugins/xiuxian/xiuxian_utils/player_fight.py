...
def Player_fight(player1: dict, player2: dict, type_in, bot_id):
    ...
    user1_hp_buff = Decimal(user1_main_buff_data['hpbuff'] if user1_main_buff_data is not None else 0)
    user1_mp_buff = Decimal(user1_main_buff_data['mpbuff'] if user1_main_buff_data is not None else 0)
    try:
        user_1_impart_data = xiuxian_impart.get_user_info_with_id(player1['user_id'])
    except:
        user_1_impart_data = None
    user_1_impart_hp = Decimal(user_1_impart_data['impart_hp_per'] if user_1_impart_data is not None else 0)
    user_1_impart_mp = Decimal(user_1_impart_data['impart_mp_per'] if user_1_impart_data is not None else 0)
    ...
    user2_hp_buff = Decimal(user2_main_buff_data['hpbuff'] if user2_main_buff_data is not None else 0)
    user2_mp_buff = Decimal(user2_main_buff_data['mpbuff'] if user2_main_buff_data is not None else 0)
    try:
        user_2_impart_data = xiuxian_impart.get_user_info_with_id(player2['user_id'])
    except:
        user_2_impart_data = None
    user_2_impart_hp = Decimal(user_2_impart_data['impart_hp_per'] if user_2_impart_data is not None else 0)
    user_2_impart_mp = Decimal(user_2_impart_data['impart_mp_per'] if user_2_impart_data is not None else 0)
    ...
    player1['气血'] = player1['气血'] * (1 + user1_hp_buff)
    player2['气血'] = player2['气血'] * (1 + user2_hp_buff)
    player1['真元'] = player1['真元'] * (1 + user1_mp_buff)
    player2['真元'] = player2['真元'] * (1 + user2_mp_buff)
                    ...
                    if isEnableUserSikll(player1, user1_hp_cost, user1_mp_cost, player1_turn_cost,
                        ...
                        if user1_skill_type == 1:  # 直接伤害类技能
                            ...
                            if user1_buff_type == 1:  # 攻击类buff
                                ...
                                play_list.append(
                                    get_msg_dict(player1, player1_init_hp,
                                                 msg1.format(player1['道号'], number_to(player1_sh))))
                    ...
                    if isEnableUserSikll(player2, user2_hp_cost, user2_mp_cost, player2_turn_cost,
                        ...
                        if user2_skill_type == 1:  # 直接伤害类技能
                            ...
                            if user2_buff_type == 1:  # 攻击类buff
                                ...
                                play_list.append(
                                    get_msg_dict(player2, player2_init_hp,
                                                 msg2.format(player2['道号'], number_to(player2_sh))))
...
async def Boss_fight(player1: dict, boss: dict, type_in=2, bot_id=0):
    ...
    if Items().convert_rank('混沌境初期')[0] < Items().convert_rank((boss["jj"] + '中期'))[0] < \
            Items().convert_rank('江湖好手')[0]:  # 混沌以下
        boss["减伤"] = 1  # boss减伤率
        boss_zs = 0
        boss_hx = 0
        boss_bs = 0
        ...
        boss_jb = 0
        boss_xl = 0
    if Items().convert_rank('创世境初期')[0] < Items().convert_rank((boss["jj"] + '中期'))[0] < \
            Items().convert_rank('斩我境圆满')[0]:  # 混沌境
    ...
    if Items().convert_rank('金仙境初期')[0] < Items().convert_rank((boss["jj"] + '中期'))[0] < \
            Items().convert_rank('混沌境圆满')[0]:  # 创世境
    ...
    if Items().convert_rank('轮回境初期')[0] < Items().convert_rank((boss["jj"] + '中期'))[0] < \
            Items().convert_rank('创世境圆满')[0]:  # 金仙境
    ...
    if Items().convert_rank('虚神境初期')[0] < Items().convert_rank((boss["jj"] + '中期'))[0] < \
            Items().convert_rank('金仙境圆满')[0]:  # 轮回境
    ...
    if Items().convert_rank('仙帝境初期')[0] < Items().convert_rank((boss["jj"] + '中期'))[0] < \
            Items().convert_rank('轮回境圆满')[0]:  # 虚神境
    ...
    if Items().convert_rank('祭道境初期')[0] < Items().convert_rank((boss["jj"] + '中期'))[0] < \
            Items().convert_rank('虚神境圆满')[0]:  # 仙帝境
        boss["减伤"] = 0.1  # boss减伤率
        boss_st1 = random.randint(0, 100)  # boss神通1
        if 0 <= boss_st1 <= 25:
            boss_zs = 0.9  # boss攻击
        ...
        if boss_turn_skip:
            ...
            if boss_sub <= 8:
                ...
                play_list.append(get_boss_dict(boss, qx, msg2.format(boss['name'], number_to(
                    Decimal(boss_sh) * (1 + Decimal(boss_zs)) * 5 + (Decimal(player1['气血']) * Decimal(0.3)))),
                                               bot_id))
                player1['气血'] = player1['气血'] - ((Decimal(
                    boss_sh * (1 + Decimal(boss_zs)) * (Decimal(player1_js) - Decimal(random_def)) * 5) + (
                                                              Decimal(player1['气血']) * Decimal(0.3))))
                play_list.append(
                ...
                play_list.append(get_boss_dict(boss, qx, msg2.format(boss['name'], number_to(
                    Decimal(boss_sh) * (1 + Decimal(boss_zs)) * (
                            Decimal(player1_js) - Decimal(random_def) + Decimal(0.5)) * 3)), bot_id))
                player1['气血'] = player1['气血'] - (
                    (Decimal(boss_sh * (1 + boss_zs) * (player1_js - random_def + 0.5) * 3)))
                play_list.append(
                    get_boss_dict(boss, qx, f"{player1['道号']}剩余血量{number_to(player1['气血'])}", bot_id))
                ...
                if isCrit:
                    ...
                    msg2 = "{}发起攻击，造成了{}伤害\n"
                play_list.append(
                    get_boss_dict(boss, qx, msg2.format(boss['name'], number_to(boss_sh * (1 + boss_zs))), bot_id))
