...
async def draw_user_info_img(user_id, DETAIL_MAP):
    ...
    #     # 贴一层黑色遮罩
    #     img.paste(i := Image.new("RGBA", (based_w, based_h), (0, 0, 0, 100)), mask=i)
    # except:
    ...
    # temp = await get_avatar_by_user_id_and_save(user_id)  # zzy改  不调用下载qq头像的方法，因为获取到的是虚拟id
    temp = Image.open(TEXT_PATH / 'init.png').resize((280, 280)).convert("RGBA")
